...
for(i in 1:length(n.seq)){
  ...
  for(i in 1:length(n.seq)){
    n <- 2^n.seq[i]
  #  load(paste0('data/logistic/logisticInits', '_n', n, '.RData'))
   # Par.fn <- function(){
    #  list(ln_r = unname(log(inits[1])), ln_K = unname(log(inits[2])), 
     #      ln_sig = unname(log(inits[3])), ln_tau = unname(log(inits[4])), 
      #     u = unname(inits[5:length(inits)]))
    #}
    Mod <- 'logistic'
    simdata <- gendat(seed=i,
                      N=n,
    ...
    # adnuts.mod <- sample_nuts('logisticGrowth', cores=1, init = Par.fn)
    #                           iter = 2000, 
    #                           init = Par.fn,
    #                           chains = 1,
    #                         #  warmup = 2000,
