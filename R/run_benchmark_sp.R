...
#Set up #spp/#factors
#no.fix parm = c()
mod.dim <- list(m1=c(3,6), m2=c(5,10), m3=c(10,20), m4=c(20,40))
n <- 100
for(i in 1:length(mod.dim)){
  seed=i
  set.seed(seed)
  nf <- mod.dim[[i]][1]
  nj <- mod.dim[[i]][2]
  nl <- nj*nf-(nf*(nf-1))/2
  Spp.b0 <- runif(nj,1,5)
  Loadings <- rnorm(nl, 0, 1)
  simdata <- gendat(seed=i,
                    N=NA,
                    theta = c(50,0.75), #c(Range,sp.var)
                    u1 = NA,
                    var = NA,
                    mod.name = 'spatial_mspp',
                    mspp.dim = mod.dim[[i]],
                    spp.b0 = Spp.b0,
                    loadings = Loadings)
  set.seed(i)
  samp.idx <- sample(1:nrow(simdata), n)
  sampdata <- simdata[samp.idx,]
  save(Spp.b0, Loadings, simdata, sampdata, file = paste0('data/spatial_mspp/spatial_mspp_m', i, '.RData'))
  Mod <- 'spatial_mspp'
  
  spatial.results <- runTMB(dat = sampdata[1:(2+nj)], Mod, prType = 1, nf=nf)
  save(spatial.results, file = paste0('results/spatial/spatial_mspp_m',i,'.RData'))
}
