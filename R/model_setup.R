...
runTMB <- function(dat,mod,seed=123,prType=0, nf = NULL){
  results <- list()
  if(mod != 'spatial' & mod != 'spatial_mspp'){
  ...
  if(mod == 'spatial_mspp'){
    setupTMB(dll.name = 'spatial_poisson_mspp')
    inits <- mkSpatialInits(dat,0,'tmb_mspp', nf)
    Dat <- inits$Dat
    Par <- inits$Par()
    Random = 'omega'
    upr.limit <- rep(Inf, length(unlist(Par)))
  ...
  if(mod == 'spatial_mspp'){
    results$tmb <- list(par.est = c(summary(sdr,'fixed')[,1]),#,summary(sdr,'report')[,1]),
                        se.est = c(summary(sdr,'fixed')[,2]),#,summary(sdr,'report')[,2]),
                        omega.est = summary(sdr, 'random')[,1],
                        time = as.numeric(difftime(b,a, units = 'mins')), 
                        stan.time = NA, meanESS = NA, minESS = NA)
  }
  ...
  if(is.null(summary(tmbstan.mod))){
    if(mod != 'spatial'){
      ...
      if(mod == 'spatial_mspp'){
        n.par <- length(Par$b0) + length(Par$loadings)
        results$tmbstan <- list(par.est = rep(NA, n.par),
                                se.est = rep(NA, n.par),
                                omega.est = rep(NA, nrow(mon-1-n.par)),
                                time = as.numeric(difftime(b,a, units = 'mins')), 
    ...
    if(mod == 'spatial_mspp'){
      n.par <- length(Par$b0) + length(Par$loadings)
      results$tmbstan <- list(par.est = c(summary(tmbstan.mod)[[1]][1:n.par,1] ),
                              se.est = summary(tmbstan.mod)[[1]][1:n.par,2],
                              omega.est = c(summary(tmbstan.mod)[[1]][(n.par+1):(nrow(mon)-1)]),
                              time = as.numeric(difftime(b,a, units = 'mins')), 
                              meanESS = mean(mon$Bulk_ESS), minESS = min(mon$Bulk_ESS))
      warning('sigma and tau SE in log space, not comparable to other model runs')
    }
...
runADMB <- function(dat,mod){
  ...
  if(mod == 'logistic'){
    models <- c('logisticGrowth', 'logisticGrowth_PQL', 'logisticGrowth_nonsep')
    results <- list()
    for(m in 1:length(models)){
      mod.name <- models[m]
      Dat <- list(n=length(dat), y = dat)
      if(mod.name == 'logisticGrowth_PQL'){
        Par <- list(ln_r = log(0.5), ln_K = log(80), ln_sig=log(sqrt(0.01)),ln_tau=-1,  u = rep(1,n) )
      } else {
      ...
      a <- Sys.time()
      admb.mod <- run_admb(mod.name, verbose = TRUE, extra.args = '-mno 5000000')
      b <- Sys.time()
